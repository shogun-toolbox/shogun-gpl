	...
	while (!done && iter <= options.max_iter)
	{
		...
		for (task=0; task<n_tasks; task++)
		{
			...
			for (int i=0; i<n_vecs_task; i++)
			{
				...
				Fs += (std::log(std::exp(-bb) + std::exp(aa-bb)) + bb)/n_vecs_task;
				double b = -y[task_idx[i]]*(1 - 1/(1+std::exp(aa)))/n_vecs_task;
				gCs[task] += b;
		...
		while (inner_iter <= 1)
		{
			...
			for (task=0; task<n_tasks; task++)
			{
				...
				for (int i=0; i<n_vecs_task; i++)
				{
					...
					Fzp += (std::log(std::exp(-bb) + std::exp(aa-bb)) + bb)/n_vecs_task;
				}
			}
