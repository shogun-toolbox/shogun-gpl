...
int32_t QPproblem::optimal()
{
  ...
  else
  {
      ...
      if (fabs(kktold-aux) < delta*0.01 &&  aux < delta*2.0)
      {
          if (DELTAvpm > InitialDELTAvpm*0.1)
          {
              ...
              SG_INFO("Inner tolerance changed to: {}\n", DELTAvpm)
          }
      }
  ...
  *****************************************************************************/
      ...
      if (verbosity > 1)
      {
          SG_INFO("  Working set: new components: {}", kin)
          SG_INFO(",  new parameter n: {}\n", q)
      }
...
int32_t QPproblem::Preprocess1(sKernel* kernel, int32_t *aux, int32_t *sv)
{
  ...
  if (verbosity > 0)
  {
      SG_INFO("  Preprocessing: examples = {}", s)
      SG_INFO(", subp. = {}", n)
      SG_INFO(", size = {}\n",sl)
  }
...
float64_t QPproblem::gpdtsolve(float64_t *solution)
{
  ...
  do
  {
    ...
    if (verbosity > 1)
        ...
        SG_INFO("  {:8.2f} |", (float64_t)t/CLOCKS_PER_SEC)
    ...
    do {
      ...
      if (tau_proximal < 0.0)
      {
        ...
        if ((-dfval/aux) < -0.5*tau_proximal)
        {
          tau_proximal_this = -tau_proximal;
          if (verbosity > 0)
            SG_DEBUG("tau threshold: {}  ", -dfval/aux)
        }
      }
      proximal_count++;
  ...
  SG_INFO("- Objective function value: {}\n", fval)
