		...
		)
{
	...
	while (ncbm.exitflag==0)
	{
		...
		/* Verbose output */
		if (verbose)
			io::print("{:4d}: tim={:.3f}, Fp={}, Fd={}, (Fp-Fd)={}, (Fp-Fd)/Fp={}, R={}, nCP={}, nzA={}, QPexitflag={}, best_fp={}, gap={}\n",
		...
		for (size_t i = 0; i < wbest_candidates.size(); i++)
		{
			...
			if (!is_convex)
			{
				...
				if (score > best_risk)
				{
					...
					if (verbose)
						io::print("CONFLICT Rbest={:.6g} score={:g} L={:.6g} U={:.6g}\n", best_risk, score, L, U);
					if (L <= U)
					{
						if (verbose)
							io::print("{:.6f} < {:.6f} => changing bias[{}]={:g}\n", L, U, cp_idx, L);
						bias[cp_idx]= -L;
					}
					else
					{
						...
						if (verbose)
							io::print("solved by changing nCP={} bias:{:g} ({:g})\n", cp_idx, bias[cp_idx], L);
					}
				}
			}
		}
